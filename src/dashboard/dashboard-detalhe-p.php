    ...
    if(isset($_POST['fr_dte']) && $_POST['fr_dte'] == 'mes atual'){
        ...
        $selapp = "
            SELECT bd.user as user,
                round(ifnull(sum((julianday(strftime('%Y-%m-%d %H:%M', (julianday(strftime('%Y-%m-%d %H:%M', ul.logdte || ' ' || ul.to_logtim)) - julianday(((24.0 - 23.0) / 24.0) *3.0)))) - julianday(strftime('%Y-%m-%d %H:%M', (julianday(strftime('%Y-%m-%d %H:%M', ul.logdte || ' ' || ul.fr_logtim)) - julianday(((24.0 - 23.0) / 24.0) *3.0))))) *24 * 1), 0), 2) as aponta
