    ...
    if(isset($_POST['fr_adddte']) && isset($_POST['to_adddte'])){
        if(isset($_POST['usr_id']) && !empty($_POST['usr_id'])){
            ...
            */
            $selapp = "
                ...
                FROM (SELECT
                            ...
                            (dt.shftim * 1)*(1-".$fadiga.") as shftim,
                            us.usr_id as user
                        FROM 
                            yr_idx dt 
                            join 
            ...
            /*
            Fim do apontamento entre duas datas com usuário - Sem indice
            */

            /*
            Inicio da disponibilidade entre duas datas com usuário - Sem indice
            */
            $seldsp = "
                select sum(dispon) as indisp from (
                    SELECT bd.days,
                            ...
                            bd.shftim,
                            uf.funidx,
                            ifnull(sum((julianday(ul.to_logtim) - julianday(ul.fr_logtim)) *24 * 1), 0) as aponta,
                            ifnull(sum((julianday(ui.to_logtim) - julianday(ui.fr_logtim)) *24 * 1), 0) as indisp,
                            round((bd.shftim - 
                            ifnull(
                                    sum(
                                    (julianday(strftime('%Y-%m-%d %H:%M', (julianday(strftime('%Y-%m-%d %H:%M', ul.logdte || ' ' || ul.to_logtim)) - julianday(((24.0 - 23.0) / 24.0) *3.0)))) 
                                    - julianday(strftime('%Y-%m-%d %H:%M', (julianday(strftime('%Y-%m-%d %H:%M', ul.logdte || ' ' || ul.fr_logtim)) - julianday(((24.0 - 23.0) / 24.0) *3.0))))) 
                                    *24 * 1), 0)
                             - ifnull(sum((julianday(ui.to_logtim) - julianday(ui.fr_logtim)) *24 * 1), 0)), 2) as dispon
            ...
            /*
            Fim da disponibilidade entre duas datas com usuário - Sem indice
            */
        }
        else{
            /*
            ...
            */
            ...
            /*
            Fim do apontamento entre duas datas - Sem indice
            */

            /*
            Inicio da disponibilidade entre duas datas - Sem indice
            */
            $seldsp = "
                select sum(dispon) as indisp from (
                    SELECT bd.days,
            ...
            /*
            Fim da disponibilidade entre duas datas - Sem indice
            */
        }
    }
    else {
...
<div role="main" class="container-fluid">
    <div class="card">
        <div class="card-body" <?php print $tamanho; ?>>
            <form method="POST" action="dashboard-normal.php">
                <div class="row">
                    <div class="col">
